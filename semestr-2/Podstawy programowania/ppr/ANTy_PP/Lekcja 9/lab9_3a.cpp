...
*/
    fwrite(&dana, sizeof(rekord), 1, out);
...
/*
   fscanf(in, "%c", &dana.plec);
   fscanf(in, "%i", &dana.wiek);
   fscanf(in, "%f", &dana.wzrost);
   fscanf(in, "%s", dana.imie);
*/
    fread(&danaBis, sizeof(rekord), 1, out);
   fclose(in);
