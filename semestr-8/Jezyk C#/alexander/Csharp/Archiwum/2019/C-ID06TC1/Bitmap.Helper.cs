...
namespace BitmapHelper
{
    class BitmapArray
    {
        unsafe static public Bitmap ArrToBmp(int[,] arr)
        {
            ...
            unsafe
            {
                ...
                for (int y = 0; y < Y; ++y, ptr += data.Stride - 3 * X)
                {
                    ...
                    {
                        Color clr = Color.FromArgb(arr[y, x]);
                        ptr[2] = clr.R;
                        ptr[1] = clr.G;
                        ptr[0] = clr.B;
                    }
        ...
        unsafe static public int[,] BmpToArr(Bitmap bmp)
        {
            ...
            unsafe
            {
                ...
                for (int y = 0; y < Y; ++y, ptr += data.Stride - 3 * X)
                {
                    for (int x = 0; x < X; ++x, ptr += 3)
                    {
                        arr[y, x] = Color.FromArgb(255,ptr[2], ptr[1], ptr[0]).ToArgb();
                    }
                }
            }
            bmp.UnlockBits(data);
