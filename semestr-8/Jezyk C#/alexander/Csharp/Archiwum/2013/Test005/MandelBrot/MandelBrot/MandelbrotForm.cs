...
namespace Mandelbrot
{
    public partial class MandelBrotForm : Form
    {
        ...
        private uint Work = 0;
        private double FXmin = 0.2016, FYmin = 0.541, FXmax = 0.2175, FYmax = 0.5521;
        ...
        private void recalc()
        {
            ...
            (
                ...
                0.2016, 0.541, 0.2175, 0.5521
            );
        }

        private void calc(int Xmax, int Ymax,
                    double FXmin, double FYmin, double FXmax, double FYmax)
        {
            Stop = false;
            ++Work;
            Bitmap bmp = new Bitmap(Xmax, Ymax);
            for (int y = 0; (y < Ymax) && (!Stop) ; ++y)
            {
                double Pi = ((Ymax - y) * FYmin + (y - 0) * FYmax) / (Ymax - 0);
                for (int x = 0; (x < Xmax) && (!Stop) ; ++x)
                {
                    double Pr = ((Xmax - x) * FXmin + (x - 0) * FXmax) / (Xmax - 0);
                    int i = 0;
        ...
        {
            Stop = true;
            while (Work>0) Application.DoEvents();
            recalc();
        }

        private Bitmap calcBmp(double Lf, double Tp, double Rt, double Bt)
        {
            Bitmap bmp = new Bitmap(128, 128);
            for (int y = 0; y < 128; ++y)
            {
                double Pi = ((128 - y) * Tp + (y - 0) * Bt) / (128 - 0);
                for (int x = 0; x < 128; ++x)
                {
                    double Pr = ((128 - x) * Lf + (x - 0) * Rt) / (128 - 0);
                    int i = 0;
                    for (double Zr = 0, Zi = 0; i < 255; ++i)
                    {
                        double Tmp = Zr * Zr - Zi * Zi + Pr;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            Bitmap all = new Bitmap(W, H);
            Graphics g = Graphics.FromImage(all);

            double Tp = FYmin;
            for (int y = 0; y < H; y += 128)
            {
                double Y = y + 128, Bt = ((H - Y) * FYmin + (Y - 0) * FYmax) / (H - 0);
                double Lf = FXmin;
                for (int x = 0; x < W; x += 128)
                {
                    double X = x + 128, Rt = ((W - X) * FXmin + (X - 0) * FXmax) / (W - 0);
                    
                    //MessageBox.Show(String.Format("{0} {1} {2} {3}", Lf, Tp, Rt, Bt));
                    g.DrawImage(calcBmp(Lf, Tp, Rt, Bt), x, y);
                    Lf = Rt;
                }
                Tp = Bt;
            }
