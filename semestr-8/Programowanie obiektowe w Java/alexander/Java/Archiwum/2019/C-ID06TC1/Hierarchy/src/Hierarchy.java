...
public class Hierarchy extends javax.swing.JFrame
{
   ...
   public Hierarchy()
   {
      ...
      al.addAll(Arrays.asList(tb));

      //for(int i=0;i<al.size();++i) al.add(al.get(i));
      //for(Animal a:al) hs.add(a);
      //al.forEach((a) -> hs.add(a));
      hs.addAll(al);

      //for(Animal i:hs) LbModel.addElement(i);
      //hs.forEach((i) -> LbModel.addElement(i));
      LbModel.addAll(hs); // nie możliwe

      for(Animal a:LbModel) CbModel.addElement(a); // nie możliwe
      LbModel.forEach((a) -> CbModel.addElement(a)); // nie możliwe
